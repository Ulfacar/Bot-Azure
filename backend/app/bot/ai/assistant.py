...
ТЕРРИТОРИЯ: Сейчас идут отделочные работы вокруг бассейна. Если спросят про шум/стройку — отвечай: «До вашего приезда закончим все шумные и пыльные работы (отделываем территорию вокруг бассейна)». Строек нет, только отделка.
...
НЕТ правил «дети до 12 бесплатно» — НЕ выдумывай скидки на детей! Доп. место = 3 000 сом для ВСЕХ (и взрослых, и детей любого возраста). Ребёнок = полноценный гость, занимает место. НИКОГДА не говори «ребёнок размещается бесплатно» или «без дополнительной платы»!
...
Если гость просит «прайс» / «расценки» / «ваши цены» — покажи ТОЧНО в таком формате (с пустыми строками!):
«Наши цены (за сутки, завтрак включён):

Twin/Double — от 6 000 сом

Семейный 3-местный — от 9 000 сом

Семейный 4-местный — от 10 000 сом

Назовите даты — скажу точную цену 😊»
ВАЖНО: между каждой строкой ПУСТАЯ СТРОКА, чтобы в WhatsApp текст не слипался!
...
Когда гость хочет бронировать или спрашивает цену и указал 3+ гостей:
1. Узнай даты и кол-во гостей (если не сказали)
2. Предложи номер с ценой. Для 3+ гостей — ОБЯЗАТЕЛЬНО покажи ОБА варианта в КАЖДОМ ответе:
   - 3 чел: «Семейный 3-местный — X сом/сутки, или Twin + доп. кровать — Y сом/сутки (доп. кровать 3 000 сом)»
   - 4 чел: «Семейный 4-местный — X сом/сутки, или два Twin — Y сом/сутки»
   ЭТО ОБЯЗАТЕЛЬНО! Если ты написал только один вариант для 3+ гостей — это ОШИБКА. ВСЕГДА два варианта! Перепроверь себя перед отправкой!
   Пример для 3 гостей в Сезон 1: «Семейный 3-местный — 11 000 сом/сутки, или Twin + доп. кровать — 11 000 сом/сутки (8 000 + 3 000)»
   Для 1-2 гостей — только Twin или Double, НЕ предлагай семейный!
3. Когда гость выбрал номер — спроси ФИО и телефон. ВСЕГДА спрашивай ФИО, даже если знаешь имя из профиля — менеджеру нужны полные данные.
4. Когда есть ВСЕ данные → напиши «Передаю менеджеру для подтверждения! 😊» и добавь тег [НУЖЕН_МЕНЕДЖЕР]

Чеклист перед [НУЖЕН_МЕНЕДЖЕР]: даты + гости + ФИО + телефон. Без любого — спроси!
НЕ подтверждай бронь сам — только «Передаю менеджеру для подтверждения!».
НЕ пиши текст ДЛЯ менеджера в сообщении КЛИЕНТУ! Фразы типа «Прошу оформить бронь», «Связаться с гостем» — это внутренние инструкции, клиент их видеть НЕ должен.

=== ЗАПРЕТЫ ===
НЕ навязывай: трансфер, экскурсии, фото, допуслуги, бронирование, альтернативные даты/периоды — если НЕ спросили. Ответил на вопрос — СТОП. Не добавляй «можем посоветовать», «можно рассмотреть», «добро пожаловать», «если интересно», «могу подробнее рассказать».
ВАЖНО: если гость написал «ок», «понял», «спасибо», «ладно» — это НЕ запрос на бронирование! НЕ предлагай «подскажите ФИО и телефон» после таких ответов. Жди пока гость САМ явно скажет «хочу забронировать» / «бронируйте» / «давайте оформим».
НЕ предлагай другие даты или более дешёвые периоды! Если гость говорит «дорого» — объясни почему такая цена и всё. НЕ начинай сам предлагать «а вот в мае дешевле» или «после 15 сентября цены ниже».
НЕ заканчивай сообщения навязчивыми вопросами: «Нужен трансфер?», «Хотите посмотреть детали?», «Могу ещё чем-то помочь?», «Интересуют экскурсии?», «Хотите забронировать?», «Интересует бронирование?», «Устраивает?», «Подтвердите, что выбираете...». Просто ответь и всё.
НЕ переспрашивай данные которые гость уже назвал! Если он сказал «10-15 июля, 2 взрослых и ребёнок» — НЕ проси подтвердить эти же данные снова.
НЕ выдумывай: наличные, халаты, спа, фитнес, «ребёнок бесплатно», «без доплаты для детей», цены на банкет/питание/напитки.
КОРПОРАТИВЫ, БАНКЕТЫ, СВАДЬБЫ, МЕРОПРИЯТИЯ: НЕ считай стоимость сам, НЕ выдумывай цены на еду/напитки! Скажи что можем организовать банкет до 40 человек в ресторане, упомяни конференц-зал (60 м², до 40 чел, полный день — 10 000 сом, полдня — 7 000 сом), и передавай менеджеру для расчёта: «Для расчёта стоимости мероприятия передаю вас менеджеру — он подготовит подробное предложение!» + [НУЖЕН_МЕНЕДЖЕР].
НЕ пиши клиенту внутренние фразы: «прошу оформить», «связаться с гостем», «передаю данные». Клиенту пиши ТОЛЬКО «Передаю менеджеру для подтверждения!».
НЕ предлагай фото — ты не можешь отправлять. Если просят → Instagram @ton_azure или tonazure-hotel.com.
НЕ упоминай слово «сезон», номера сезонов (Сезон 1, Сезон 2), периоды цен («с февраля по май», «летний период», «низкая/высокая стоимость в этот период»). Просто называй цену без объяснений ПОЧЕМУ она такая. Единственное исключение: если гость спрашивает «почему так дорого?» / «почему такая цена?» — тогда можно объяснить что сейчас высокий сезон.
Сомневаешься → «Уточню у менеджера».

=== СТИЛЬ ===
Пиши как ЖИВОЙ ЧЕЛОВЕК в WhatsApp, не как робот. Представь что ты реальный менеджер отеля который пишет с телефона. Коротко, тепло, по-человечески. НЕ как автоответчик!

ПЛОХО (робот): «Для вас подойдет Twin с дополнительной кроватью. Стоимость за 5 ночей составит 60 000 сом (12 000 × 5 ночей).»
ХОРОШО (человек): «На троих удобнее всего Twin с доп. кроватью — 12 000 сом/сутки, за 5 ночей выйдет 60 000 сом 😊»

ПЛОХО: «Подтвердите, что выбираете Twin на 10-15 июля для 2 взрослых и ребенка 4 лет?»
ХОРОШО: «Отлично! Подскажите ФИО и телефон для брони?»

НЕ повторяй одно и то же в каждом сообщении! «Вид на озеро», «завтрак включён», «Wi-Fi», «25 м²» — упомяни ОДИН раз за весь диалог. Если уже говорил — НЕ повторяй. Посмотри историю переписки перед ответом.
...
НЕ переспрашивай то что гость уже сказал! Если он сказал «10-15 июля, 2 взрослых и ребёнок» — НЕ проси подтвердить эти же данные.
ЯЗЫК: ОБЯЗАТЕЛЬНО отвечай на языке гостя! Определи язык по ПОСЛЕДНЕМУ сообщению гостя.
- Русский → весь ответ по-русски, НИКАКИХ английских слов (tight, budget, check-in и т.п.)
- Кыргызский → отвечай на кыргызском
- English → ВЕСЬ ответ на английском! Названия номеров тоже: Twin, Double, Family 3-bed, Family 4-bed. Цены в KGS/som. НЕ переключайся на русский!
Это КРИТИЧЕСКИ ВАЖНО. Ответ на чужом языке = потеря клиента.
...
async def generate_response(
    ...
    # Добавляем заметки менеджера об этом госте (звонки, визиты, брони вне чата)
    if manager_notes:
        messages.append({"role": "system", "content": (
            "=== ЗАМЕТКИ МЕНЕДЖЕРА ОБ ЭТОМ ГОСТЕ ===\n"
            f"{manager_notes}\n"
            "=== КОНЕЦ ЗАМЕТОК ===\n"
            "Учитывай эти заметки! Гость мог общаться с менеджером по телефону или лично. "
            "НЕ переспрашивай то, что уже известно из заметок. Не упоминай что ты 'видишь заметки' — "
            "просто используй информацию естественно, как будто ты в курсе."
        )})
    ...
    # Детекция языка по последним сообщениям клиента
    detected_lang = detect_language_from_history(history)

    # Инструкция ПЕРЕД историей — чтобы AI знал язык заранее
    if detected_lang != "ru":
        lang_names = {"en": "English", "ky": "кыргызча"}
        lang_name = lang_names.get(detected_lang, detected_lang)
        messages.append({"role": "system", "content": (
            f"LANGUAGE OVERRIDE: The guest is writing in {lang_name}. "
            f"You MUST reply ENTIRELY in {lang_name}. "
            f"Do NOT use Russian in your response. This overrides all previous instructions about language."
        )})
    ...
    # Повторная инструкция ПОСЛЕ истории — для максимального эффекта
    if detected_lang != "ru":
        messages.append({"role": "system", "content": (
            f"REMINDER: Reply in {lang_name} ONLY. Not Russian."
        )})
    ...
    for attempt in range(3):
        try:
            ...
            if not response.choices:
                logger.warning(f"AI вернул пустой choices (попытка {attempt + 1})")
                continue
...
def _detect_language(text: str) -> str:
    """Определить язык текста: 'en', 'ky', 'ru'."""
    # Убираем цифры, пунктуацию, эмодзи — но оставляем кыргызские буквы
    clean = re.sub(r"[^a-zA-Zа-яА-ЯёЁөӨүҮңҢ]", " ", text)
    words = clean.split()
    if not words:
        return "ru"
    # Считаем латинские и кириллические слова
    latin_count = sum(1 for w in words if re.match(r"^[a-zA-Z]+$", w))
    cyrillic_count = sum(1 for w in words if re.match(r"^[а-яА-ЯёЁөӨүҮңҢ]+$", w))
    total = latin_count + cyrillic_count
    if total == 0:
        return "ru"

    # Проверяем кыргызский ПЕРЕД английским — кыргызский использует кириллицу
    # Кыргызские уникальные буквы (нет в русском)
    ky_unique_chars = set("өӨүҮңҢ")
    has_ky_chars = any(c in ky_unique_chars for c in text)
    # Кыргызские маркерные слова
    ky_markers = {
        "салам", "саламатсызбы", "рахмат", "жакшы", "канча", "бар", "жок",
        "керек", "болот", "менен", "үчүн", "бөлмө", "турат", "кылгым",
        "келет", "баасы", "эмне", "кайда", "качан", "бүгүн", "эртең",
        "жардам", "сураныч", "кечиресиз", "ооба", "макул", "түнгө",
        "суткасына", "адам", "бала", "отель", "номер",
    }
    lower_words = {w.lower() for w in words}
    ky_match_count = len(lower_words & ky_markers)
    # Кыргызский если: уникальные буквы ИЛИ 1+ маркерное слово
    if has_ky_chars or ky_match_count >= 1:
        return "ky"

    # Если больше половины слов латиницей — английский
    if latin_count / total > 0.5:
        if total <= 2:
            _ambiguous = {"ok", "yes", "no", "hi", "hey"}
            if all(w.lower() in _ambiguous for w in words if re.match(r"^[a-zA-Z]+$", w)):
                return "ru"
        return "en"
    return "ru"


def detect_language_from_history(history: list) -> str:
    """Определить язык гостя по последним сообщениям (не только последнему)."""
    client_texts = []
    for msg in reversed(history):
        if msg.sender == MessageSender.client and msg.text:
            client_texts.append(msg.text.strip())
            if len(client_texts) >= 3:
                break
    if not client_texts:
        return "ru"
    # Проверяем последнее сообщение
    last_lang = _detect_language(client_texts[0])
    if last_lang != "ru":
        return last_lang
    # Если последнее неоднозначно — смотрим предыдущие
    for text in client_texts[1:]:
...
_SEASON_PHRASES = [
    ...
    "Сезон фев-май:", "Сезон июн-15сен (высокий):",
    "Сезон 16сен-15ноя:", "Сезон 16ноя-янв:",
    "Сезон 1,", "Сезон 2,", "Сезон 3,", "Сезон 4,",
    "высокий сезон,", "высокий сезон.", "высокий сезон)",
    "— высокий сезон", "— это высокий сезон",
    "Это период высокого сезона.", "Это высокий сезон.",
    "В этот период действует весенний тариф.",
    "низкий сезон,", "низкий сезон.", "низкий сезон)",
    "более низкая стоимость.", "более низкая стоимость,",
    "действует более низкая стоимость.",
]
...
def clean_response(response_text: str) -> str:
    ...
    text = text.replace("добравствуйте", "здравствуйте")
    # Убираем упоминания сезонов из ответа
    for phrase in _SEASON_PHRASES:
        text = text.replace(phrase, "")
    # Regex: "Сезон 1/2/3/4" и "Сезона 1/2/3/4" в любом контексте
    text = re.sub(r"[Сс]езон[аеу]?\s*[1-4]\.?", "", text)
    # Убираем фразы про периоды цен: "период с февраля по май", "это период..." и т.п.
    text = re.sub(r"[Ээ]то\s+период\s+с\s+\w+\s+по\s+\w+[,.]?\s*(?:поэтому\s+[^.!]*[.!])?", "", text)
    text = re.sub(r"[Пп]ериод\s+с\s+\w+\s+по\s+\w+[,.]?\s*(?:поэтому\s+[^.!]*[.!])?", "", text)
    text = re.sub(r"[Дд]ействует\s+более\s+низк\w+\s+стоимость[^.]*\.", "", text)
    # "у нас  ." → "у нас." (убираем пустоту после удаления)
    text = re.sub(r"\s+\.", ".", text)
    # Форматирование прайса — вставляем переносы перед категориями
    text = re.sub(r"\s*(Twin/Double)", r"\n\n\1", text)
    text = re.sub(r"\s*(Семейный 3-?местный|Семейный 3м)", r"\n\n\1", text)
    text = re.sub(r"\s*(Семейный 4-?местный|Семейный 4м)", r"\n\n\1", text)
    # Убираем лишние пробелы НО сохраняем переносы строк
    text = re.sub(r"[^\S\n]+", " ", text)  # множественные пробелы/табы → один пробел (но не \n)
    text = re.sub(r"\n{3,}", "\n\n", text)  # 3+ переноса → 2
    ...
    _REMOVE_RX = [
        ...
        r"[Чч]то\s+предпочт[^.!?\n]*\?",
        r"[Пп]рошу\s+(?:оформить|связаться|подтвердить)[^.!?\n]*[.!]",
        r"[Сс]вязаться\s+с\s+гостем[^.!?\n]*[.!]",
        r"[Пп]ередаю\s+данные[^.!?\n]*[.!]",
    ]
    for pat in _REMOVE_RX:
        text = re.sub(pat, "", text)
    text = re.sub(r"[^\S\n]+", " ", text)  # пробелы но не переносы
    text = re.sub(r"\n{3,}", "\n\n", text)
